...
void myCanny(const Mat&src,Mat&output,int kernalsize,int sigma,int max,int min);
void myHoughLines (const Mat& input,Mat& output,double d_rho, double d_theta,int threshold);
...
int T[4][4]={{0,-1,0,1},
             ...
             {-1,-1,1,1}};
/*Debug记录：
 * 1...区分无梯度和梯度=0两个东西，否则有问题的
 * 2...记得类似卷积的操作要用一个临时的图接住...不要直接在被卷的图上改...铁定出错。
 * 3...梯度的方向...看模板可以确定梯度正方向才能正确的分区。
 */
Mat raw;
...
//        capture.read(raw);
        ...
        /****************我的Canny****************/
        myCanny(gray,gray,3,1,9,3);//Sobel用60-20
        imshow("myCanny",gray);

        /**************和官方函数比较一下*************/
//        double  time=getTickCount();
//        Canny(gray,gray,80,40);
//        cout<<"FPS:"<<getTickFrequency()/(getTickCount()-time)<<endl;
//        imshow("Canny",gray);


        /***************我的霍夫线变换***************/
        myHoughLines(gray,line_img,1,CV_PI/180,100);
        imshow("line_img",raw);


        /***************我的霍夫圆变换***************/
        myHoughCircles(gray,circle_img,150,20,2,10,30,20);
    }
}
...
void myHoughLines (const Mat& input,Mat& output,double d_rho, double d_theta,int threshold)
{
    ...
    int theta_num=(CV_PI/d_theta),hr=pow(rows*rows+cols*cols,0.5)/d_rho-1,rho_num=2*hr+1;
    ...
    for (int i = 0; i < theta_num; ++i) {
        for (int j = 0; j <rho_num ; ++j) {
            if(scoretable.ptr<float>(i)[j]>threshold)
            {
