...
void HoughCirc(Mat input, Mat & output, Mat src) {
	...
	// Find Peaks
	for (int r = 0; r < rMax; r++) for (int rho = 0; rho < rhoMax; rho++) for (int theta = 0; theta < thetaMax; theta++) {
				if (argFlag[r][rho][theta] == 0) continue;
				int &rrhot = argCnt[r][rho][theta];
				int maxCnt = rrhot;
				if (r > 0) maxCnt = MAX(maxCnt, argCnt[r - 1][rho][theta]);
				if (r < rMax - 1) maxCnt = MAX(maxCnt, argCnt[r + 1][rho][theta]);
				if (rho > 0) maxCnt = MAX(maxCnt, argCnt[r][rho - 1][theta]);
				if (rho < rhoMax - 1) maxCnt = MAX(maxCnt, argCnt[r][rho + 1][theta]);
