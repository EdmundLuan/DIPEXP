...
void HoughLine(Mat input, Mat &output) {
	...
	rhoMax = (int)sqrt(input.rows * input.rows + input.cols * input.cols);
	thetaMax = 360;
	threshold = 30;

	typedef std::vector<int> veci;
	veci zeros(rhoMax, 0);
	std::vector<veci> argCnt(thetaMax, zeros);
	...
	for (int i = 0; i < input.rows; i++)
		for (int j = 0; j < input.cols; j++) {
			for (int theta = 0; theta < thetaMax; theta++) {
				int rho = (int)(cos(2 * pi / thetaMax * theta) * i + sin(2 * pi / thetaMax * theta) * j + 0.5);
				++argCnt[theta][rho];
				if (argCnt[theta][rho] > threshold) {
					rhos.push_back(rho);
